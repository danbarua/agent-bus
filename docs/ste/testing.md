...
A positive test can pass without exercising the fix it is meant to prove. `leave()` falls back to `os.getpid()` when no pid is given. A positive test with the correct pid exercises this fallback, not the fix.
...
**The tell:** when a fix corrects a wrong input instead of rejecting it, write two tests. Test that the wrong input is now safe. Test that the common case, an absent input, stays silent. Either test alone covers only half the fallback chain.
...
A `leave()` regression test flaked at about 60-70% because of this zombie behavior. The manual probe method (see above) showed the process died correctly. Only the test's own liveness check was wrong.
...
Instrumenting `run_listen` with a log line found the cause. On a failing run, the listener's own log was empty. The process died before its first `print()`. That is evidence about the code under test, not about the test framework.

**The tell:** when a hypothesis is about the test framework, and a first such hypothesis has already failed, stop guessing. Instrument the code under test directly.
...
`tests/agent_bus/test_conventions.py` enforces a narrower set of these rules. Each entry there has its own enforcing test beside it.
