...
- **generic (unknown harness)** — no MCP, no hooks. CLI `listen` + `watch`,
  driven from its shell. This is the shape any unrecognised harness falls
  back to; `pi` was the fixture that once exercised it in
  `tests/agent_bus/integration/` and has since been culled (no adapter needs
  a proof-of-concept to demonstrate a fallback), so this row is no longer
  measured by an automated run -- the findings below stand as history.
...
| | Claude Code | Codex | Grok Build | omp | generic |
|---|---|---|---|---|---|
...
Claude and Grok woke and acted. Codex, omp, and a generic CLI-only harness all answered `NO_MONITOR`.
...
Codex and a generic CLI-only harness have neither, measured. Both have a
shell, so both could in principle block on a read of `watch` — untested, and
not claimed here.
