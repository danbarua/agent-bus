...
def test_inbox_is_serialized_identically_by_both_surfaces(bus, holder, capsys):
    ...
    assert main(["inbox", "--address", "sender", "--json"]) == 0
    via_cli = json.loads(capsys.readouterr().out)
...
def test_text_output_paths_render(bus, holder, capsys):
    ...
    assert main(["inbox", "--address", "target"]) == 0
    ...
    mid = json.loads(_dump(main, capsys, ["inbox", "--address", "target", "--json"]))[0]["id"]
    assert main(["read", mid[:8], "--address", "target"]) == 0
