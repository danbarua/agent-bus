...
peer's first act is to start `watch --target <me>`, and watch exits 1 with
...
def _kill_watch_for(name: str) -> None:
    """Kill every live `agent-bus watch --target {name}`, by pid, wherever it
    ...
    """
    r = subprocess.run(["pgrep", "-f", f"watch --target {name}"],
...
def mail_woken_peer(name: str, brief: str, *, harness: str, env: dict[str, str],
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            proc.kill()
        # `agent-bus watch --target {name}` by name, separately (#185). Checked
