...
def start_uds_listen(name: str, host_pid: int, home: str | None = None) -> int | None:
    ...
    # Routine diagnostics go to the structured log (agent_bus.log), not here
    # -- this file only ever receives an uncaught traceback or log.configure's
    # own stderr fallback, so an empty file means the listener is healthy,
    # not silent.
    #
