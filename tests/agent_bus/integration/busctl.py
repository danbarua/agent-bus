...
def inbox(home, name, *, isolate_native=True):
    ...
    either.
    """
    r = bus(home, "inbox", "--json", "--address", name, isolate_native=isolate_native)
    if r.returncode != 0:
        raise AssertionError(f"inbox --address {name} failed: {r.stderr.strip()}")
    try:
        return json.loads(r.stdout or "[]")
    except json.JSONDecodeError as e:
        raise AssertionError(f"inbox --address {name} gave non-JSON: {r.stdout!r}") from e
