...
def delivered(bus, holder, capsys):
    ...
    assert main(["inbox", "--target", READER, "--json"]) == 0
...
def test_the_watch_line_carries_an_id_the_reader_can_act_with(delivered, capsys):
    ...
    assert _out(capsys, ["ack", shown, "--target", READER]).strip() == "marked read"
...
def test_a_reader_can_get_one_whole_message_by_id(delivered, capsys):
    ...
    full_id, _ = delivered
    assert BODY.strip() in _out(capsys, ["read", full_id, "--target", READER])


def test_a_reader_can_read_by_the_prefix_the_notice_gave(delivered, capsys):
    full_id, _ = delivered
    assert BODY.strip() in _out(capsys, ["read", full_id[:8], "--target", READER])
...
def test_the_inbox_carries_whole_messages(bus, holder, capsys):
    ...
    shown = _out(capsys, ["inbox", "--target", READER])
...
def test_the_whole_cycle_works_from_the_text_output_alone(delivered, capsys):
    ...
    assert BODY.strip() in _out(capsys, ["read", ident, "--target", READER])
    assert _out(capsys, ["ack", ident, "--target", READER]).strip() == "marked read"
...
def test_acking_the_same_message_twice_is_not_an_error(delivered, capsys):
    ...
    full_id, _ = delivered
    assert _out(capsys, ["ack", full_id[:8], "--target", READER]).strip() == "marked read"
    assert _out(capsys, ["ack", full_id[:8], "--target", READER]).strip() == "marked read"
...
def test_one_pass_of_the_cycle_creates_no_files_in_the_working_tree(
    ...
    _out(capsys, ["inbox", "--target", READER])
    _out(capsys, ["read", full_id[:8], "--target", READER])
    _out(capsys, ["ack", full_id[:8], "--target", READER])
