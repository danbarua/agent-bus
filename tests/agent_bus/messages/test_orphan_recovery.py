...
from agent_bus.protocol import AgentTarget, MessageId, now_iso
...
def test_stranded_mail_is_readable_by_its_address_alone(bus):
    ...
    got = [m["text"] for m in store.get_inbox(
        target=AgentTarget("claude:4684f35f-4f8b-407a-a56a-6b436cf2f000"), home=bus)]
    assert got == ["one", "two"]
...
def test_an_unknown_target_is_an_error_not_an_empty_inbox(bus):
    ...
    with pytest.raises(ValueError, match="no such agent"):
        store.get_inbox(target=AgentTarget("nobody-at-all"), home=bus)
...
def test_recovered_mail_can_also_be_acked(bus):
    ...
    msgs = store.get_inbox(target=AgentTarget("claude:sid-ack"), home=bus)
    assert store.ack_message(MessageId(msgs[0]["id"]), target=AgentTarget(
        "claude:sid-ack",
    ), home=bus) is True
    assert store.get_inbox(
        target=AgentTarget("claude:sid-ack"), unread_only=True, home=bus) == []
...
def test_all_seven_messages_become_readable(bus):
    ...
    for eid, _, texts in REAL_SHAPE:
        got = [m["text"] for m in store.get_inbox(target=AgentTarget(eid), home=bus)]
...
def test_the_id_is_recovered_from_the_message_not_the_filename(bus):
    ...
    assert [m["text"] for m in store.get_inbox(target=AgentTarget(weird), home=bus)] == ["hello"]
...
def test_an_adopted_entry_is_pruned_once_its_mail_is_read(bus):
    ...
    store.adopt_orphan(o, home=bus)
    msgs = store.get_inbox(target=AgentTarget("claude:sid-x"), home=bus)
    store.ack_message(MessageId(msgs[0]["id"]), target=AgentTarget("claude:sid-x"), home=bus)
...
def test_a_mailbox_with_a_live_owner_is_not_an_orphan(bus):
    ...
    try:
        ...
        store.send_message(to=entry.id, text="hi", from_name=AgentTarget("s"), home=bus)
        assert store.find_orphaned_inboxes(bus) == []
    finally:
        holder.kill()
        holder.wait()
