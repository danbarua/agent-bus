...
1. Run this bash command and print its output verbatim:
   {{cli}} inbox --target {{driver}} --json > {{evidence}}/inbox.json ; cat {{evidence}}/inbox.json
2. The output is a JSON array holding exactly one message. Find its "id"
   ...
   {{cli}} read MSG_ID --target {{driver}} --json > {{evidence}}/read.json ; cat {{evidence}}/read.json
3. Using the same MSG_ID, run this bash command and print its output
   verbatim:
   {{cli}} ack MSG_ID --target {{driver}} --json > {{evidence}}/ack.json ; cat {{evidence}}/ack.json
4. Run this bash command and print its output verbatim:
   {{cli}} inbox --target {{driver}} --json > {{evidence}}/inbox_after.json ; cat {{evidence}}/inbox_after.json
5. Print DONE.
