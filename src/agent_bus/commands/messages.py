...
def inbox(
    ...
    """Messages addressed to `address`, or to whoever we are.
    ...
    """
    msgs = store.get_inbox(name_or_id=address, unread_only=unread_only, home=home)
    return [message_to_json(m) for m in msgs]


@logged
def read_one(message_id: str, address: str | None = None,
    ...
    """
    msgs = inbox(address=address, unread_only=False, home=home)
...
@logged
def ack(message_id: str, address: str | None = None, home: str | None = None) -> dict[str, Any]:
    return {"acked": bool(store.ack_message(message_id, name_or_id=address, home=home))}
